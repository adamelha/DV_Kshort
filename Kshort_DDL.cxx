...
//----------------------------------------------------------------------------
  ...
  // pi+ tracks
  const xAOD::TrackParticleContainer* recoTracks_piplus = 0;
  CHECK( evtStore()->retrieve( recoTracks_piplus, "InDetTrackParticles" ) );
  if ((*recoTracks)->TrackParticle()->m()==0.1396 && (*recoTracks)->TrackParticle()->charge()==1 ){
      recoTracks_piplus = *recoTracks;  
  }
  
 
  // pi- tracks
  const xAOD::TrackParticleContainer* recoTracks_piminus = 0;
  CHECK( evtStore()->retrieve( recoTracks_piminus, "InDetTrackParticles" ) );
  if ((*recoTracks)->TrackParticle()->m()==0.1396 && (*recoTracks)->TrackParticle()->charge()==-1 ){
      recoTracks_piminus = *recoTracks;
  } 
